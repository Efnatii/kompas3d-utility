...
public static class KompasPagesBridge
{
    ...
    public static KompasBridgeStatus GetStatus()
    {
        ...
        try
        {
            ...
            if (app == null)
            {
                ...
                result.ErrorMessage = "KOMPAS.Application.7 is not running.";
                return result;
            }
    ...
    public static KompasOpenDocumentResult OpenDocument(string documentPath)
    {
        ...
        try
        {
            ...
            {
                throw new FileNotFoundException("Document was not found.", documentPath);
            }

            IApplication app = GetOrStartApplication();
            app.Visible = true;
            IKompasDocument document = app.Documents.Open(documentPath, false, true);
            if (document == null)
            {
                throw new InvalidOperationException("Documents.Open returned null.");
            }

            result.Success = true;
            result.DocumentName = SafeString(delegate() { return document.Name; });
            result.DocumentPath = SafeString(delegate() { return document.PathName; });
            return result;
        }
        catch (Exception ex)
        {
            result.Success = false;
            result.ErrorCode = "open_document_failed";
            result.ErrorMessage = ex.Message;
            return result;
        }
    }
        ...
        string[][] matrix)
    {
        if (cellWidthMm <= 0)
        {
            throw new ArgumentOutOfRangeException("cellWidthMm", "cellWidthMm must be positive.");
        }

        if (cellHeightMm <= 0)
        {
            throw new ArgumentOutOfRangeException("cellHeightMm", "cellHeightMm must be positive.");
        }

        int rows = matrix == null ? 0 : matrix.Length;
        if (rows <= 0)
        {
            throw new InvalidOperationException("Matrix rows must be greater than zero.");
        }

        int cols = matrix.Max(row => row == null ? 0 : row.Length);
        if (cols <= 0)
        {
            throw new InvalidOperationException("Matrix cols must be greater than zero.");
        }
        ...
        IDrawingTable drawingTable = drawingTables.Add(
            rows,
            cols,
            cellHeightMm,
            cellWidthMm,
        ...
        {
            throw new InvalidOperationException("DrawingTables.Add returned null.");
        }

        ITable table = (ITable)drawingTable;
        if (table == null)
        {
            throw new InvalidOperationException("DrawingTable cast to ITable returned null.");
        }
