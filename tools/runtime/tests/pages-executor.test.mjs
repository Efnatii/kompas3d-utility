...
} from "../../../docs/assets/executor-shell.js";
import {
  buildAutoOutputPath,
...
} from "../../../docs/assets/modules/xlsx-to-kompas-tbl.js";
...
test("fallback bridge helpers target temp asset path and preview DrawingTables failures", () => {
  assert.equal(
    buildFallbackBridgeScriptPath("C:\\Temp"),
    "C:\\Temp\\kompas-pages\\xlsx-to-kompas-tbl.bridge.ps1",
  );
  assert.equal(
    buildFallbackBridgeArguments("C:\\Temp\\kompas-pages\\xlsx-to-kompas-tbl.bridge.ps1", "export"),
    '-NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\\Temp\\kompas-pages\\xlsx-to-kompas-tbl.bridge.ps1" -Action "export"',
  );
  assert.equal(
    shouldUseFallbackBridge(new Error("Public member 'DrawingTables' on type 'IView' not found.")),
    true,
  );
  assert.equal(
    shouldUseFallbackBridge(new Error("Adapter invoke surface 'kompas' is not registered.")),
    false,
  );
});
